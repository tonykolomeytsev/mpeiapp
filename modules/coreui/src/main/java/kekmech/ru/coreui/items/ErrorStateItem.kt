...
private class ErrorStateItemBinder(
    ...
    private fun getErrorStateContent(throwable: Throwable?): Triple<Int, Pair<Int, String?>, Int> =
        when (throwable) {
            ...
            ->
                Triple(
                    R.string.common_error_no_internet_title,
                    R.string.common_error_no_internet_description to null,
                    R.drawable.ill_no_internet_error
                )
            is HttpException ->
                Triple(
                    R.string.common_error_bad_response_title,
                    R.string.common_error_bad_response_description to throwable.code().toString(),
                    R.drawable.ill_bad_response_error
                )
                    .takeIf { throwable.code() in 300 until 600 }
                    ?: getErrorStateContent(null)
            is SSLHandshakeException ->
                Triple(
                    R.string.common_error_handshake_title,
                    R.string.common_error_handshake_description to null,
                    R.drawable.ill_handshake_error
                )
            is CompositeException ->
                getErrorStateContent(throwable.exceptions.firstOrNull())
            else ->
                Triple(
                    R.string.common_error_view_title,
                    R.string.common_error_view_description to null,
                    R.drawable.ill_view_error
                )
        }
}