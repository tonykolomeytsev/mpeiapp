...
class InMemoryCache(
    ...
    fun <T : Any> put(key: String, value: T) = getSubject<T>(key).onNext(Optional.of(value))

    fun remove(key: String) {
        cache[key]?.onNext(Optional.empty())
    }

    fun <T : Any> peek(key: String): T? = getSubject<T>(key).value?.orElse(null)
    ...
    fun <T : Any> get(key: String): Maybe<T> = Maybe.create<T> { emitter ->
        getSubject<T>(key).value?.orElse(null)?.let(emitter::onSuccess) ?: emitter.onComplete()
    }.observeOn(io())
    ...
    private fun <T : Any> updateInternal(key: String, newValue: (previousValue: T?) -> T?) {
        ...
        synchronized(subject) {
            newValue(subject.value?.orElse(null))?.let { subject.onNext(Optional.of(it)) }
        }
    }
